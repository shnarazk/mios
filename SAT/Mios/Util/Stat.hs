...
instance Monoid MiosConfiguration where
  mempty = MiosConfiguration 0 0 False 0 0 0 0
  mappend a b
    | a == b = a
...
instance Dumpable MiosConfiguration where
  header = (intercalate "," ["vDecay", "propLim", "debug", "par1", "par2", "par3", "par4"], mempty)
  toCSV (MiosConfiguration vd pl dp p1 p2 p3 p4) = intercalate "," [show vd, show (logBase2 pl), show (show dp), showf p1, showf p2, show p3, show p4]
  average = mconcat
...
fromStat = do
  ...
  return $
    MiosDump
      (so, MiosConfiguration (read va) (read pr) False 0 0 0 0)
...
fromCSV = do
  ...
  return $
    MiosDump
        (so, MiosConfiguration (read vd) (2 ^ ((read pl) :: Int)) (read dp) (read p1) (read p2) (read p3) (read p4))
        (CNFDescription (Just fl) (read nv) (read nc))
        (MiosStats (sort
