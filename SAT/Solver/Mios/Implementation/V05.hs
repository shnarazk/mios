...
--
module SAT.Solver.Mios.Implementation.V05 where
...
-- | sets to the version name : @"mios v0.5 #pointer"@
idString :: String
idString = "mios v0.5 #thatWorksReally"
...
instance ContainerLike (FixedVecOf a) a where
  ...
  dump str FixedVecOf{..} = return $ str ++ "FixedVecOf"
...
instance VectorLike (FixedVecOf a) a where
  ...
  (.!) FixedVecOf{..} n = MV.read fVec n
  {-# SPECIALIZE INLINE setAt :: FixedVecOf a -> Int -> a -> IO () #-}
  setAt FixedVecOf{..} n x = MV.write fVec n x
  -- * Duplication
...
instance VectorLike FixedLitVec Lit where
  ...
  (.!) FixedLitVec{..} n = UV.read litVec n
  {-# SPECIALIZE INLINE setAt :: FixedLitVec -> Int -> Lit -> IO () #-}
  setAt FixedLitVec{..} n x = UV.write litVec n x
  -- * Duplication
